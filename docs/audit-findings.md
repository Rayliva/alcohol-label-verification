...
| # | Status | Finding |
|---|---|---|
| C1 | **open** | `anthropic.APIError` derives from `Exception`, not from `OSError`/`RuntimeError`/`ValueError`, so every provider outage, 429, 401 and timeout falls past the 502 handler in `api/routes.py` to the generic 500 backstop |
