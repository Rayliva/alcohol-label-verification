...
| D3 | **fixed** | Test counts wrong throughout: README said 285 and 166 rule-engine, `build-loop.md` 283. Corrected then; drifted again and re-corrected 2026-08-12 (see S10). The counts move; a stated count carries its date now |
...
| D5 | **fixed** | `CLAUDE.md`: "`OCR_ENGINE=fake` runs the whole stack offline" — false twice over. CLAUDE.md now states fake OCR still requires `ANTHROPIC_API_KEY` to check a label and that only the test suite is credential-free (verified 2026-08-12) |
...
| D8 | **fixed** | `samples/README.md` and README pointed at `samples/labels/`; the 31 labels live in `api/app/samples/`. Main README corrected earlier; `samples/README.md` and the generator docstring corrected 2026-08-12 |
...
## Session 6, 2026-08-12 — pre-submission audit, four parallel reviewers

Requirements, security, logic, and staleness dimensions, run before turn-in.

| # | Status | Finding |
|---|---|---|
| S4 | **fixed** | **The client-side results CSV had no formula escaping.** The server's batch export defuses `=`/`+`/`-`/`@` cells because detected values are OCR of attacker-printable artwork; the single-label export built in the browser (ResultsScreen.toCsv) was missed. Same `'`-prefix escaping now, with a unit test |
| S5 | **fixed** | **Batch uploads had no size or count cap.** Every image was read fully into memory before preflight; `MAX_UPLOAD_BYTES` guarded only `/api/verify`. Now 25 MB per image and 400 images per batch, refused with a typed reason |
| S6 | **fixed** | **`/api/verify` buffered the whole body before its size check.** A multi-gigabyte request spent the full transfer before the 400. A `Content-Length` precheck refuses grossly oversized requests up front; the post-read check remains for length-less senders |
| S7 | **fixed** | **Review routes were guarded per-route, not structurally.** A route added to `review_routes.py` would have been open by default, against the stated include-time-guard design. Login and logout now live on their own public router; everything else is guarded at include time |
| S8 | **accepted** | **No throttle on `/api/login`; sessions not revocable before their 12-hour expiry.** Both are the cost of storing no credential or session state. Recorded in the README's Known limitations |
| S9 | **accepted** | **OCR text reaches the extraction model**, so artwork can carry printed text aimed at the model. Bounded by the schema and the deterministic rule engine; recorded in the README's Known limitations |
| S10 | **fixed** | **README test counts had drifted again** (285/423/18 across three places against 438 backend, 31 frontend measured today), an echo of D3; "fill in the two credentials" undersold the four required values, an echo of D2. All recounted and corrected 2026-08-12 |
| S11 | **fixed** | **`samples/generate_results.py` docstring still said `samples/labels`**, a D8 residue. It reads `api/app/samples` and says so now |
| S12 | **fixed** | **Any navigation out of the submit step stranded a running batch.** The hidden-not-unmounted protection only covered the mode toggle; opening a review, checking a single label, or going home unmounted BatchScreen and lost the job id while the server kept spending provider calls with no Stop and no export. SubmitScreen is now hidden, never unmounted, while signed in |
| S13 | **fixed** | **A double-click on "Check these N labels" started two identical jobs**, checking every label twice and queueing every result twice. The button disables and reads "Starting…" while the upload is in flight |
| S14 | **fixed** | **A restart mid-batch froze the progress screen silently forever.** Every poll 404ed and the error was swallowed, leaving "47 of 200" on screen while polling continued indefinitely. A server-sent error now surfaces and stops the poll; a network blip still retries quietly |
| S15 | **fixed** | **A failed check's error banner followed the agent to every screen** until the next submit cleared it. Navigation clears it now |
| S16 | **fixed** | **Seeded queue order was the reverse of the manifest** within each verdict bucket: `-received_at` ranking negated the offsets meant to preserve it. Seeds now carry negative offsets, with a unit test pinning uploads-first, manifest-order-behind |
| S17 | **fixed** | **Batch API calls sent no credentials and signing out mid-check left the request running.** Cross-origin development 401ed on every batch call; a sign-out during processing landed the next sign-in on the previous session's results. Batch fetches are credentialed and sign-out aborts like going home does |
| S18 | **accepted** | **"Start reviewing" starts at the queue's first undecided item, ignoring any active search or filter.** Deliberate: the run follows the queue's one order. Revisit if agents expect filtered runs |
| S19 | **accepted** | **A batch's full reports live twice in memory** (job store and review queue), roughly 42 MB per 300-label run, accumulating until restart. Recorded in the README beside the queue-growth note |
| S20 | **fixed** | **Dead API surface from removed UI features**: the `reviewer` form field and response field (the initials box was removed; nothing sent or read it), `FieldOutcome.override` (typed always-None since per-field decisions were removed), and QueueScreen's duplicate outcome-label map (now `verdictWord`, which was exported and unused). `/api/beverage-types` and `/api/batch/{id}/label/{index}` stay deliberately, each with its reason in its docstring |
| S21 | **fixed** | **Doc drift across six documents**: build-loop CURRENT STATE was a session behind; tech-spec still claimed no auth, a Vercel frontend, and engaged prompt caching; ui-spec described the beverage selector, PDF accept, per-field overrides, the initials field, self-hosted fonts, and batch row-click as current (superseded markers added; the doc is chronological by design); PRD P1-10 described the removed per-field override and tier 1 as AI-generated; `samples/README.md` pointed at the phantom `samples/labels/`; `.env.example` omitted `INSECURE_COOKIES` |
